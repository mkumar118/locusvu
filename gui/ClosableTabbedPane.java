...
public class ClosableTabbedPane extends JTabbedPane {
	...
	private ArrayList[] m_arrayOfLists;

	public ClosableTabbedPane(GUI gui){
		super();
		this.m_gui = gui;
	...
	/**
	 * this class has been modified, see method changes
	 * @author mkumar
	 *
	 */
	private class TabCloseUI implements MouseListener, MouseMotionListener {
		...
		/**
		 * this method has been modified to all remove the respective table and array
		 * from all the 'listOf____' lists
		 *
		 */
		...
		/**
		 * this method removes the array from the 'listOf___' list
		 * and removes the table from the 'listOfTables'
		 */
		private void removeFromLists(){
			m_arrayOfLists = m_gui.getArrayOfLists();
			
			// remove from array
			for(int i=0; i < m_arrayOfLists.length; i++){
				m_arrayOfLists[i].remove(selectedTab);
			}			
		}
